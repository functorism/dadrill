import { constant, emptyArray, pipe } from "./function";
...
export type Rewrite<A extends JsonValue> = {
  match: (json: JsonValue) => json is A;
  toJson: (a: A) => JsonValue;
  fromJson: (a: A) => (json: JsonValue) => A;
};

export const Rewrite = <A extends JsonValue>(
  // TODO: Should probably be parse not validate
  match: (json: JsonValue) => json is A,
  toJson: (a: A) => JsonValue,
  fromJson: (a: A) => (json: JsonValue) => A
): Rewrite<A> => ({
...
export const drillFromJson = ({
  ...
  if (isJsonObject(json)) {
    ...
    for (const rewriter of rewriters) {
      if (rewriter.match(json)) {
        const rewritten = rewrite(
          // @ts-ignore
          rewriter.toJson(json),
          rewriter.fromJson(json),
          json,
          lens,
          path
